...
// raw SDL event for button presses on a controller also from the poll event loop
// then translated by an input translator
// example usage:
...
// the contained data is a coordinated based on the relative position of the mouse on the screen
// example usage
// SomeQueue.enqueue(RawCursorEvent(Event))
