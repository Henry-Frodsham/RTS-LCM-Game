...
// since an entity could exist without an ogre component its neccessary that is standalone
// example usage:
// SomeQueue.enqueue(CreateEntityEvent(ERef, "myNode", Ogre::Vector3(0.f,0.f,0.f))
// then
...
struct CreateEntityEvent {
	...
	//so its neccessary that it persists in the heap until subsequent events (storing this shared ptr) are also destroyed
...
// example usage:
// SomeQueue.enqueue(AddOgreComponentEvent(ERef, "myNode", Ogre::Vector3(0.f,0.f,0.f))
// then
...
struct AddOgreComponentEvent {
	...
	AddOgreComponentEvent(std::shared_ptr<entt::entity> ent, std::string NodeN,Ogre::Vector3 InitP)
		: Entity(ent)
		, NodeName(NodeN)
...
// example usage:
// SomeQueue.enqueue(AddMeshComponentEvent(ERef, "myNode", Ogre::Vector3(0.f,0.f,0.f))
// then
