...
TEST_CASE("ECS - constructor") {
    CHECK_NOTHROW(ErrorReporter Obj());
}
...
TEST_CASE("ECS - entity with mesh creation") {
    ...
    RenderSystem& RS = RenderSystem::GetInstance();
    RS.Init();
    WM->WorldQueue->Enqueue(CreateMeshWorldEntityEvent("test2","Sphere.mesh","test45345ent",Ogre::Vector3(0.5f, 0.f, -5.f))); 
