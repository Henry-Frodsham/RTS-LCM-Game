...
TEST_CASE("InputListener - succesful translation") {
  ...
  SDL_KeyboardEvent MockEvent = SDL_KeyboardEvent{
      0, 0, 0, 0,
      0, 0, 0, SDL_Keysym{SDL_Scancode::SDL_SCANCODE_B, SDL_KeyCode::SDLK_b}};

  IT.WaitingEvents->Enqueue(RawKBEvent(MockEvent, false));

  IT.Update(0.f);

  CHECK(IT.GetNumPressedKeys() != 0);  // NOLINT [readability/check]
}

TEST_CASE(
    "InputListener - succesful erasure of key press when the key is released") {
  RenderSystem& RS = RenderSystem::GetInstance();
  InputListener Obj = InputListener(RS.GetSDLWindow());
  InputDevice* MockDevice = new InputDevice(nullptr, InputDeviceType::KBM);
