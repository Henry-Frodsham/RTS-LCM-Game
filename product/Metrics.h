...
// contains metric information for an input device which is then displayed on
// the overlay
struct InputMetric {
  int NumPressedKeys;
  float AxisY;
  ...
  InputMetric(int Pkeys, float AxY, float AxX)
      : NumPressedKeys(Pkeys), AxisY(AxY), AxisX(AxX) {}

  bool operator==(const InputMetric& Other) const {
    return NumPressedKeys == Other.NumPressedKeys && AxisY == Other.AxisY &&
           AxisX == Other.AxisX;
  }
};
